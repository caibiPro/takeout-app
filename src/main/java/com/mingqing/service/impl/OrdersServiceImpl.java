...
import com.mingqing.entity.*;
import com.mingqing.mapper.OrdersMapper;
import com.mingqing.service.*;
...
public class OrdersServiceImpl extends ServiceImpl<OrdersMapper, Orders> implements OrdersService {
	...
	public void submit(Orders orders) {
		...
		List<OrderDetail> orderDetails = shoppingCarts.stream().map(cartItem -> {
			...
			amount.addAndGet(cartItem.getAmount().multiply(new BigDecimal(cartItem.getNumber())).intValue());
			return orderDetail;
		...
		order.setAddress((address.getProvinceName() == null ? "" : address.getProvinceName()) + (address.getCityName() == null ? "" : address.getCityName()) + (address.getDistrictName() == null ? "" : address.getDistrictName()) + (address.getDetail() == null ? "" : address.getDetail()));
